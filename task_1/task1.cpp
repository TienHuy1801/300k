...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        if (strcmp(argv[i], "-n") == 0)
        {
            if (++i < argc)
            {
                ...
                {
                    logOutFile << "Error 01: expecting number after '-n' argument.\n";
                    return 1;
                }
            }
            else
        ...
        else if (strcmp(argv[i], "-st") == 0)
        {
            if (++i < argc)
            {
                try
                {
                    ...
                    if (sampling < 1)
                    {
                        logOutFile << "Error 02: minimum sample rate is 1.\n";
                        return 1;
                    }
                }
                catch (const exception &e)
                {
                    logOutFile << "Error 01: expecting number after '-st' argument.\n";
                    return 1;
                }
            }
            else
        ...
        else if (strcmp(argv[i], "-si") == 0)
        {
            if (++i < argc)
            {
                try
                {
                    ...
                    if (interval < 1)
                    {
                        logOutFile << "Error 02: minimum simulation interval is 1.\n";
                        return 1;
                    }
                }
                catch (const exception &e)
                {
                    logOutFile << "Error 01: expecting number after '-si' argument.\n";
                    return 1;
                }
            }
            else
        ...
        {
            logOutFile << "Error 03: unknown argument '" << argv[i] << "'.\n";
            return 1;
        }
    }
    ...
    uniform_real_distribution<double> dist{0.0, 600.0};
